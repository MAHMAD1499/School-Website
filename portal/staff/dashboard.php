...
$body=json_decode(file_get_contents('php://input'),true)??[];if($isAjax){
  ...
  $db2=ksm_db();
  $staff=$db2->query("SELECT class FROM users_staff WHERE id=$staff_id LIMIT 1")->fetch_assoc();
  ...
  ksm_json(['homework'=>$hw,'attendance'=>$att,'class'=>$class]);
}
?>
<!DOCTYPE html>
<html lang="en">
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!staff) { window.location.href = 'login.php'; return; }
