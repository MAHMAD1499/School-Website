...
$body=json_decode(file_get_contents('php://input'),true)??[];if($isAjax){
  if($method==='GET'){$r=ksm_db()->query("SELECT * FROM teachers ORDER BY id ASC");$rows=[];while($row=$r->fetch_assoc())$rows[]=$row;ksm_json($rows);}
  if($method==='POST'){$name=ksm_esc($body['name']??'');$role=ksm_esc($body['role']??'');$subject=ksm_esc($body['subject']??'');$emoji=ksm_esc($body['emoji']??'?');$bio=ksm_esc($body['bio']??'');if(!$name||!$role||!$subject)ksm_err('Name, Role, Subject required.');ksm_db()->query("INSERT INTO teachers(name,role,subject,emoji,bio)VALUES('$name','$role','$subject','$emoji','$bio')");ksm_json(['id'=>ksm_db()->insert_id],'Teacher added.');}
  if($method==='PUT'){$id=intval($body['id']??0);$name=ksm_esc($body['name']??'');$role=ksm_esc($body['role']??'');$subject=ksm_esc($body['subject']??'');$emoji=ksm_esc($body['emoji']??'?');$bio=ksm_esc($body['bio']??'');if(!$id)ksm_err('Invalid ID.');ksm_db()->query("UPDATE teachers SET name='$name',role='$role',subject='$subject',emoji='$emoji',bio='$bio' WHERE id=$id");ksm_json(null,'Updated.');}
  if($method==='DELETE'){$id=intval($_GET['id']??0);if(!$id)ksm_err('Invalid ID.');ksm_db()->query("DELETE FROM teachers WHERE id=$id");ksm_json(null,'Deleted.');}
}
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="portal-content fade-up">
      <div class="page-header">
        <div class="page-header-left">
          <h1 class="page-title">👨‍🏫 Manage Teachers & Staff</h1>
          <p class="page-subtitle">Add, edit, or remove teachers and staff members</p>
        </div>
...
<script>
  ...
  async function saveTeacher() {
    ...
    if (!name || !role || !subject) { showToast('Please fill in Name, Role, and Subject.', 'error'); return; }

    const data = { name, role, subject, emoji: document.getElementById('tEmoji').value || '👤', bio: document.getElementById('tBio').value.trim() };
